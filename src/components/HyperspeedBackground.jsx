  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      laneMarkers.forEach(marker => {
        ...
        // Pulse opacity
        marker.material.opacity = 0.4 + Math.sin(elapsedTime * 2 + marker.position.z * 0.1) * 0.2;
      });
      ...
      lights.forEach(light => {
        ...
        light.position.x += Math.sin(elapsedTime + light.position.z * 0.01) * 0.02;
        ...
        if (light.position.z > 10) {
          ...
          if (Math.random() > 0.7) {
            light.material.color.setHex(lightColors[Math.floor(Math.random() * lightColors.length)]);
            light.material.emissive.setHex(lightColors[Math.floor(Math.random() * lightColors.length)]);
          }
        }
      });
      ...
      particles.forEach(particle => {
        ...
        particle.position.y += Math.sin(elapsedTime + particle.position.x) * 0.01;
